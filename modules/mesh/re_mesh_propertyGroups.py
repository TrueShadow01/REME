...
def update_relPathToAbs(self, context):
    try:
        ...
        pass
    if (
        self.path == "" or self.path.count(".") < 2
    ):  # Check if path is empty or if number extension is missing
        self.invalid = True
    else:
        self.invalid = False
...
class ExporterNodePropertyGroup(bpy.types.PropertyGroup):
    ...
    show: BoolProperty(name="", description="", default=True)
    hasChild: BoolProperty(name="", description="", default=False)
    expand: BoolProperty(name="", description="", default=True)

    parentName: StringProperty(name="", description="", default="")
    hierarchyLevel: IntProperty(name="", description="", default=0)
    exportType: StringProperty(name="", description="", default="")
    path: StringProperty(
        name="",
        subtype="FILE_PATH",
    ...
    invalid: BoolProperty(name="", description="", default=False)
